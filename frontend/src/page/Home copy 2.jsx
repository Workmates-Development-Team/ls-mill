  ...
  const handleUpload = async () => {
    try {
      ...
      const { data } = await axios.post("http://127.0.0.1:6001/ask", {
        ...
        input_text: `Analyze the uploaded resumes to identify skill sets mentioned anywhere in the document. Compare them with the required skills '${skills}'. Each PDF's content will be sequentially numbered. Present the results in a tabular format with columns: Name, All Skills, only matched skills, Matched Percentage, and Contact Info for each resume.`,
        start_new,
        session_number,
      });
      ...
      <div className="mx-auto max-w-7xl px-2 md:px-0 h-screen pt-16 pb-28 overflow-y-auto relative chat">
        <div className="mt-3 flex flex-wrap items-center gap-4">
          {/* <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline">
                <Plus className="w-4 h-4 mr-2" /> Add Required Skills
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Write all required skills separated by commas.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="items-center gap-4">
                  <Textarea
                    ...
                    className="w-full"
                    placeholder="Write all required skills"
                    value={skills}
                  />
                </div>
              </div>
          ...
          <div>
            <Button
              disabled={
                doc_id || !selectedFiles?.length || !skills.length || loading
                  ? true
                  : false
              }
        ...
        {skills?.length ? (
          <div className="mt-3">Required skills are - {skills}</div>
        ): null}
